...
#     # Processando ambas as imagens com o iDISF
...
#     # label_img_top_tensor = torch.tensor(label_img_top)

#     # Supondo que label_img_top e label_img_side sejam seus numpy.ndarray
#     label_img_top_tensor = torch.tensor(label_img_top, dtype=torch.float32)
#     # label_img_side_tensor = torch.tensor(label_img_side, dtype=torch.float32)
#     label_img_side_tensor = torch.tensor(label_img_top, dtype=torch.float32)
    
#     # Empilhando os tensores para criar um tensor 3D
#     # combined_tensor = torch.stack([label_img_top_tensor, label_img_side_tensor], dim=0)
#     combined_tensor = torch.stack([label_img_top_tensor, label_img_side_tensor], dim=0)

#     # Convertendo o tensor PyTorch de volta para um numpy.ndarray
#     # Isso é necessário para manter a consistência com a estrutura de saída da primeira função
#     combined_array = combined_tensor.numpy()
    
#     # Para simular a estrutura da primeira função, você pode precisar realizar operações adicionais
#     # Por exemplo, se a primeira função espera um array 2D, você precisará ajustar isso
#     # Este exemplo assume que você quer manter o array 3D
    
#     # Se necessário, ajuste os valores ou faça operações adicionais aqui
#     # Por exemplo, se a saída deveria ser binária (0 ou 1)
#     combined_array = np.where(combined_array > 0, 1, 0)

#     plt.imshow(label_img_top_tensor, cmap='gray')
#     plt.colorbar()  # Opcional, mas útil para ver a escala de valores
#     plt.show()

#     #PROXIMA TAREFA: DESCOBRIR PORQUE DE label_img_top_tensor COM A SEGMENTACAO CORRETA, EM combined_array FICA TUDO PRETO
